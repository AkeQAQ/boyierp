...
export var exportLKExcel = function (luckysheet, value) {
    ...
    luckysheet.forEach(function (table) {
        ...
        if(table.config != undefined && table.config.columnlen != undefined){
            inner:
                for (let i = 0; i <1000; i++) {
                    let colWidth = table.config.columnlen[i]; // 像素
                    console.log("one sheet:",colWidth)
                    if(colWidth === null || colWidth === undefined){
                        break inner;
                    }

                    const col = worksheet.getColumn(i+1);
                    col.width = colWidth / 7.2567;  // 1宽度=7.2567px
                    console.log("像素，设置成excel宽度",colWidth,col.width)
                }
        }
...
var fontConvert = function (
    ...
    // luckysheet：ff(样式), fc(颜色), bl(粗体), it(斜体), fs(大小), cl(删除线)
    const luckyToExcel = {
        0: '微软雅黑',
        1: '宋体（Song）',
        2: '黑体（ST Heiti）',
