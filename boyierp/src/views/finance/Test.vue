  ...
  <el-container>
    <el-main class="elMain_my" style="padding-top: 0;padding-right: 0px">
      <el-form :inline="true" class="demo-form-inline elForm_my" >
        ...
        <el-form-item v-if="hasAuth('finance:test:save')">
          <el-button size="mini" icon="el-icon-plus" type="primary" v-if="hasAuth('finance:test:save')"
                     @click="add()"
        ...
        <el-form-item v-if="hasAuth('finance:test:valid')">
        ...
        <el-form-item v-if="hasAuth('finance:test:save')" style="margin-left: 0">
        ...
        >
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="edit(scope.row.id)"
                       v-if="hasAuth('finance:test:update') || (hasAuth('finance:test:list') && scope.row.status != 1 )   ">{{ scope.row.status === 1 ? '编辑' : '查看' }}
            </el-button>


            <el-button style="padding: 0" type="text"
                       v-if="hasAuth('finance:test:valid')  && (scope.row.status === 2 || scope.row.status === 3)   ">
            ...
            <el-button style="padding: 0" type="text"
                       v-if="hasAuth('finance:test:valid')  && scope.row.status ===0  ">
              <template>
            ...
            <el-divider direction="vertical"
                        v-if="hasAuth('finance:test:del')  && scope.row.status ===1  "></el-divider>

            <el-button style="padding: 0" type="text"
                       v-if="hasAuth('finance:test:del') && scope.row.status ===1   ">
              <template>
      ...
      >
        <el-form style="width: 100%;margin-bottom: -20px;margin-top: -30px;align-items: center"
          ...
          <el-form-item label="检测费日期" prop="testDate" >
          ...
          <el-form-item label="检测费单号" prop="documentNum" >
            <el-input style="width: 200px" :disabled="this.editForm.status!==1 "  v-model="editForm.documentNum">
            </el-input>
          </el-form-item>

          <el-form-item label="检测费金额" prop="testAmount" >
            <el-input style="width: 200px" :disabled="this.editForm.status!==1 "  v-model="editForm.testAmount"
          ...
          <el-form-item v-if="hasAuth('finance:test:save')">
            <el-dropdown   @command="action">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="save" v-show="hasAuth('finance:test:save') && (this.editForm.status===1 )" >
                  提交单据</el-dropdown-item>
                <el-dropdown-item command="subReturn" v-show="hasAuth('finance:test:save') && (this.editForm.status===2 || this.editForm.status===3)">
                  撤销</el-dropdown-item>
                <el-dropdown-item command="addNew" v-show="hasAuth('finance:test:save') ">
                  新增</el-dropdown-item>
                <el-dropdown-item command="copy" v-show="hasAuth('finance:test:save') ">
                  复制</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </el-form-item>
...
export default {
  ...
  methods: {
    exportCurrentList() {
      ...
      let url = '/finance/supplierTest/export?currentPage='+this.currentPage+
      ...
      request2.post(url,
        ...
        this.saveFile(blob,'供应商检测费当前页.xlsx')
      }).catch()
    },
    ...
    exportAll() {
      ...
      let url = '/finance/supplierTest/export?'+
          "searchField="+this.select+
          "&&searchStatus="+checkStr +
          "&&searchStartDate="+this.searchStartDate +
          "&&searchEndDate="+this.searchEndDate
      ...
      request2.post(url,
        ...
        this.saveFile(blob,'供应商检测费全部列表.xlsx')
      }).catch()
    },
    ...
    dialogOpend(){
      if(this.editForm.id !== '' && this.editForm.id !== undefined){
        ...
        request.get('/finance/supplierTest/lockById?id=' + this.editForm.id)
      }
    },
    ...
    statusPassBatch(){
      ...
      request.post('/finance/supplierTest/statusPassBatch',ids).then(res => {
        this.$message({
          message: '审核通过!',
          type: 'success'
        });
    ...
    submitForm(formName, methodName) {
      this.$refs[formName].validate((valid) => {
        ...
        if (valid) {
          if( this.editForm.supplierId==='' || this.editForm.testDate===''   || this.editForm.testAmount===''){
          ...
          request.post('/finance/supplierTest/' + methodName, this.editForm).then(res => {
            load.close()
            this.$message({
    ...
    getList() {
      ...
      let url = '/finance/supplierTest/list?currentPage='+this.currentPage+
          "&&pageSize="+this.pageSize+
          "&&searchField="+this.select+
          "&&searchStatus="+checkStr +
          "&&searchStartDate="+this.searchStartDate +
    ...
    edit(id) {
      this.addOrUpdate = "update"
      request.get('/finance/supplierTest/queryById?id=' + id).then(res => {
    ...
    del(id) {
      ...
      request.post('/finance/supplierTest/del', ids).then(res => {
        this.$message({
          message: '删除成功!',
          type: 'success'
        });
    ...
    // 撤销提交
    statusSubReturn(id) {
      request.get('/finance/supplierTest/statusSubReturn?id=' + id).then(res => {
        this.$message({
          message: '已撤销!',
          type: 'success'
        });
    ...
    // 状态提交
    statusSubmit(id) {
      request.get('/finance/supplierTest/statusSubmit?id=' + id).then(res => {
        this.$message({
          message: '已提交!',
          type: 'success'
        });
    ...
    // 状态待审核
    statusPass(id) {
      request.get('/finance/supplierTest/statusPass?id=' + id).then(res => {
        this.$message({
          message: '审核通过!',
          type: 'success'
        });
    ...
    // 状态反审核
    statusReturn(id) {
      request.get('/finance/supplierTest/statusReturn?id=' + id).then(res => {
        this.$message({
          message: '反审核完成!',
          type: 'success'
        });
    ...
    closeMethod(){
      ...
      if(this.editForm.id !== '' && this.editForm.id !== undefined){
        ...
        request.get('/finance/supplierTest/lockOpenById?id=' + this.editForm.id)
      }
    ...
    async closeBrowser(){
      if(this.editForm.id !== '' && this.editForm.id !== undefined){
        console.log("关闭编辑页面.打开锁...",this.editForm.id);
        await request.get('/finance/supplierTest/lockOpenById?id=' + this.editForm.id)
      }
    },
