    ...
    <el-main class="elMain_my" style="padding-top: 0;padding-right: 0px">
      ...
      >
        <el-form style="width: 100%;margin-bottom: -20px;margin-top: -30px;align-items: center"
          ...
          <el-form-item label="编号" prop="id" label-width="110px">
            <el-input style="width: 200px" :disabled=true placeholder="保存自动生成" v-model="editForm.id">
            </el-input>
          </el-form-item>

          <el-form-item label="供应商" prop="supplierName" label-width="110px">
            <!-- 搜索框 -->
            <el-autocomplete
                style="width: 200px"
          ...
          <el-form-item label="补税点日期" prop="documentDate" label-width="110px">
          ...
          <el-form-item label="开票单位" prop="company" label-width="110px">
            <el-input style="width: 200px" :disabled="this.editForm.status!==1 "  v-model="editForm.company">
            </el-input>
          </el-form-item>

          <el-form-item label="发票号" prop="documentNum" label-width="110px">
            <el-input style="width: 200px" :disabled="this.editForm.status!==1 "  v-model="editForm.documentNum">
            </el-input>
          </el-form-item>
          <el-form-item label="开票金额" prop="documentAmount" label-width="110px">
          ...
          <el-form-item label="含税未开票金额" prop="taxCalAmount" label-width="110px">
            <el-input style="width: 200px" :disabled="this.editForm.status!==1 "  v-model="editForm.taxCalAmount"
                      onkeyup="value=value.replace(/[^0-9.]/g,'')"
                      @input="changeAmount()"
          ...
          <el-form-item label="税点" prop="taxPoint" label-width="110px">
            <template slot-scope="scope">
          ...
          <el-form-item label="税额" prop="deductionAmount" label-width="110px">
            <template slot-scope="scope">
              <el-input style="width: 200px" :disabled="true"  v-model="editForm.deductionAmount"
              >
              </el-input>
            </template>
          ...
          <el-form-item label="付款状态" prop="payStatus" label-width="110px">
          ...
          <el-form-item label="付款时间" prop="payDate" label-width="110px">
...
export default {
  ...
  methods: {
    ...
    calendarChange(dates){
      ...
      request.get('/finance/supplierTaxDeduction/updatePayDate?id=' + this.editForm.id+"&&payDate="+dates).then(res => {
    ...
    statusChange(currentNum){
      if(this.editForm.payDate!='' || this.editForm.payDate!=null){
        return;
      }
      request.get('/finance/supplierTaxDeduction/updatePayStatus?id=' + this.editForm.id+"&&payStatus="+currentNum).then(res => {
        this.$message.success(res.data.data)
      }).catch(error=>{
        this.$message.error("修改失败")
        console.log("error:",error)
    ...
    changePoint(seq){
      if(this.editForm.taxCalAmount !== ''){
        this.editForm.deductionAmount = (this.editForm.taxPoint *  this.editForm.taxCalAmount).toFixed(2)
      }
    },
    changeAmount(seq){
    ...
    handleRemove(file, id) {
      ...
      request({
        method: 'get',
        url: '/finance/supplierTaxDeduction/delPic?fileName='+file.name+"&&id="+id,
    ...
    uploadRequest(fileobj) {
      ...
      request({
        method: 'post',
        url: '/finance/supplierTaxDeduction/uploadPic?id='+this.editForm.id,
        headers: {'Content-Type': 'multipart/form-data'},
        data: param
      }).then(res => {
        // 成功
    ...
    dialogOpend(){
      if(this.editForm.id !== '' && this.editForm.id !== undefined){
        ...
        request.get('/finance/supplierTaxDeduction/lockById?id=' + this.editForm.id)
      }
    },
    ...
    statusPassBatch(){
      ...
      request.post('/finance/supplierTaxDeduction/statusPassBatch',ids).then(res => {
        this.$message({
          message: '审核通过!',
          type: 'success'
        });
    ...
    submitForm(formName, methodName) {
      this.$refs[formName].validate((valid) => {
        ...
        if (valid) {
          ...
          request.post('/finance/supplierTaxDeduction/' + methodName, this.editForm).then(res => {
            load.close()
            this.$message({
    ...
    getList() {
      ...
      let url = '/finance/supplierTaxDeduction/list?currentPage='+this.currentPage+
          "&&pageSize="+this.pageSize+
          "&&searchField="+this.select+
          "&&searchStatus="+checkStr +
          "&&payStatus="+payStatus +
    ...
    edit(id) {
      this.addOrUpdate = "update"
      request.get('/finance/supplierTaxDeduction/queryById?id=' + id).then(res => {
        let result = res.data.data
        // 查看图片
        request.get('/finance/supplierTaxDeduction/getPicturesById?id='+id).then(res => {
          let data = res.data.data;
    ...
    del(id) {
      ...
      request.post('/finance/supplierTaxDeduction/del', ids).then(res => {
        this.$message({
          message: '删除成功!',
          type: 'success'
        });
    ...
    // 撤销提交
    statusSubReturn(id) {
      request.get('/finance/supplierTaxDeduction/statusSubReturn?id=' + id).then(res => {
        this.$message({
          message: '已撤销!',
          type: 'success'
        });
    ...
    // 状态提交
    statusSubmit(id) {
      request.get('/finance/supplierTaxDeduction/statusSubmit?id=' + id).then(res => {
        this.$message({
          message: '已提交!',
          type: 'success'
        });
    ...
    // 状态待审核
    statusPass(id) {
      request.get('/finance/supplierTaxDeduction/statusPass?id=' + id).then(res => {
        this.$message({
          message: '审核通过!',
          type: 'success'
        });
    ...
    // 状态反审核
    statusReturn(id) {
      request.get('/finance/supplierTaxDeduction/statusReturn?id=' + id).then(res => {
        this.$message({
          message: '反审核完成!',
          type: 'success'
        });
    ...
    closeMethod(){
      ...
      if(this.editForm.id !== '' && this.editForm.id !== undefined){
        ...
        request.get('/finance/supplierTaxDeduction/lockOpenById?id=' + this.editForm.id)
      }
      this.fileList=[]
    ...
    async closeBrowser(){
      if(this.editForm.id !== '' && this.editForm.id !== undefined){
        console.log("关闭编辑页面.打开锁...",this.editForm.id);
        await request.get('/finance/supplierTaxDeduction/lockOpenById?id=' + this.editForm.id)
      }
    },
