<template>
  <div>
    <el-form :inline="true" class="demo-form-inline" :model="editForm" >
      ...
      <el-form-item v-if="hasAuth('finance:close:save')">
        <el-button type="primary" size="mini" icon="el-icon-plus" v-if="hasAuth('finance:close:save')"  @click="submitForm">新增</el-button>
      </el-form-item>

      <el-form-item v-if="hasAuth('finance:close:return')">
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      request.post('/finance/close/save',this.editForm).then(res => {
    ...
    // 查询角色表单列表数据
    getList() {
      request.get('/finance/close/list').then(res => {
    ...
    // 查询角色表单列表数据
    returnLast() {
      request.get('/finance/close/return').then(res => {
        this.$message({
          message:'回退成功!',
          type: 'success'
        });
