...
export default {
  ...
  methods: {
    ...
    exportAll() {
      ...
      request2.post('/repository/returnMaterial/export?'+
          "searchStartDate="+this.searchStartDate+
          "&&searchEndDate="+this.searchEndDate+
          "&&searchField="+this.select
        ...
        this.saveFile(blob,'生产退料全部列表.xlsx')
      }).catch()
    },
    ...
    exportCurrent() {
      ...
      request2.post('/repository/returnMaterial/export?currentPage='+this.currentPage+
        ...
        this.saveFile(blob,'生产退料当前页.xlsx')
      }).catch()
    },
    ...
    getReturnDocumentList() {
      ...
      this.tableLoad = true;
      let checkStr = this.checkedBox.join(",");
      let url = '/repository/returnMaterial/list?currentPage='+this.currentPage+
          "&&pageSize="+this.pageSize+
          "&&total="+this.total+
          "&&searchStartDate="+this.searchStartDate+
          "&&searchEndDate="+this.searchEndDate+
    ...
    // 撤销提交
    statusSubReturn(id) {
      request.get('/repository/returnMaterial/statusSubReturn?id=' + id).then(res => {
        this.$message({
          message: '已撤销!',
          type: 'success'
        });
    ...
    // 状态提交
    statusSubmit(id) {
      request.get('/repository/returnMaterial/statusSubmit?id=' + id).then(res => {
        this.$message({
          message: '已提交!',
          type: 'success'
        });
