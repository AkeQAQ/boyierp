  ...
  <el-container>
    <el-main style="margin-top: -30px;margin-left: -30px">
      <el-form :inline="true" class="demo-form-inline" style="margin-bottom: -20px">
        ...
        <el-form-item v-if="hasAuth('externalAccount:baseData:supplierMaterial:list')">
          <el-button size="mini" icon="el-icon-plus" type="primary" v-if="hasAuth('externalAccount:baseData:supplierMaterial:list')"
        ...
        <!--
                ...
                <el-form-item v-if="hasAuth('externalAccount:baseData:supplierMaterial:del')">
        ...
        <el-form-item v-if="hasAuth('externalAccount:baseData:supplierMaterial:list')" style="margin-left: 0">
        ...
        >
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="edit(scope.row.id)"
                       v-if="hasAuth('externalAccount:baseData:supplierMaterial:list') && scope.row.status ===1  ">编辑
            </el-button>

            <el-divider direction="vertical"
                        v-if="hasAuth('externalAccount:baseData:supplierMaterial:list') && scope.row.status ===1   "></el-divider>

            <el-button style="padding: 0" type="text"
                       v-if="hasAuth('externalAccount:baseData:supplierMaterial:list')  && scope.row.status ===1   ">
              <template>
            ...
            <el-button style="padding: 0" type="text"
                       v-if="hasAuth('externalAccount:baseData:supplierMaterial:list')  && scope.row.status ===0  ">
              <template>
            ...
            <el-button type="text" size="small" @click="edit(scope.row.id)"
                       v-if="hasAuth('externalAccount:baseData:supplierMaterial:list') && scope.row.status ===0  ">修改失效日期
            </el-button>
...
export default {
  ...
  methods: {
    ...
    exportCurrentList() {
      ...
      request2.post('/externalAccount/baseData/supplierMaterial/export?currentPage='+this.currentPage+
    ...
    exportAll() {
      ...
      let checkStr = this.checkedBox.join(",");
      request2.post('/externalAccount/baseData/supplierMaterial/export?'+
          "&&searchField="+this.select+
          "&&searchStatus="+checkStr
    ...
    submitForm(formName, methodName) {
      this.$refs[formName].validate((valid) => {
        ...
        if (valid) {
          ...
          request.post('/externalAccount/baseData/supplierMaterial/' + methodName, this.editForm).then(res => {
            load.close()
            this.$message({
    ...
    async getSupplierMaterialList() {
      ...
      await request.post('/externalAccount/baseData/supplierMaterial/list?currentPage='+this.currentPage+
                "&&pageSize="+this.pageSize+
                "&&total="+this.total+
                "&&searchField="+this.select+
                "&&searchStatus="+checkStr,
    ...
    edit(id) {
      ...
      request.get('/externalAccount/baseData/supplierMaterial/queryById?id=' + id).then(res => {
    ...
    statusPassBatch(){
      ...
      request.post('/externalAccount/baseData/supplierMaterial/statusPassBatch',ids).then(res => {
        this.$message({
          message: '审核通过!',
          type: 'success'
        });
    ...
    del(id) {
      ...
      request.post('/externalAccount/baseData/supplierMaterial/del', ids).then(res => {
        this.$message({
          message: '删除成功!',
          type: 'success'
        });
    ...
    // 状态待审核
    statusPass(id) {
      request.get('/externalAccount/baseData/supplierMaterial/statusPass?id=' + id).then(res => {
        this.$message({
          message: '审核通过!',
          type: 'success'
        });
    ...
    // 状态反审核
    statusReturn(id) {
      request.get('/externalAccount/baseData/supplierMaterial/statusReturn?id=' + id).then(res => {
        this.$message({
          message: '反审核完成!',
          type: 'success'
        });
