<template>
  ...
  <el-container>
    <el-aside width="180px">
      ...
      <!-- 供应商分组按钮 -->
      <el-button type="text" size="small" v-if="hasAuth('externalAccount:baseData:supplier:list')" @click="addGroup()">新增分组</el-button>
      <el-button type="text" size="small" v-if="hasAuth('externalAccount:baseData:supplier:list')" @click="updateGroup()">修改分组
      </el-button>
      <el-button type="text" v-if="hasAuth('externalAccount:baseData:supplier:list') ">
        <!-- 气泡确认框 -->
        <template>
...
<!--                      <el-option label="子编码" value="subId"></el-option>-->
              ...
              <el-form-item v-if="hasAuth('externalAccount:baseData:supplier:list')">
                <el-button  size="mini" icon="el-icon-plus" type="primary" v-if="hasAuth('externalAccount:baseData:supplier:list')"  @click="addSupplier()">新增</el-button>
              </el-form-item>
              ...
              <el-form-item v-if="hasAuth('externalAccount:baseData:supplier:list')" style="margin-left: 0">
...
<!--              <el-table-column
              ...
              >
                <template slot-scope="scope">
                  <el-button type="text" size="small" @click="edit(scope.row.id)" v-if="hasAuth('externalAccount:baseData:supplier:list')   ">编辑</el-button>
                  <el-divider direction="vertical" v-if="hasAuth('externalAccount:baseData:supplier:list')   "></el-divider>

                  <el-button style="padding: 0" type="text"v-if="hasAuth('externalAccount:baseData:supplier:list')   ">
                    <template>
...
export default {
  ...
  methods: {
    ...
    exportCurrentList() {
      ...
      request2.post('/externalAccount/baseData/supplier/export?currentPage='+this.currentPage+
    ...
    exportAll() {
      ...
      request2.post('/externalAccount/baseData/supplier/export?'+
          "searchField="+this.select+
          "&&searchStr="+this.searchStr
          ,null
          ,{responseType:'arraybuffer'}).then(res=>{
    ...
    nodeDbClick(event,data){
      ...
      request.get('/externalAccount/baseData/supplier/listByGroupCode', {
        params: {
    ...
    submitForm(formName,methodName) {
      this.$refs[formName].validate((valid) => {
        ...
        if (valid) {
          ...
          request.post('/externalAccount/baseData/supplier/'+methodName, this.editForm).then(res => {
            load.close()
            this.$message({
      ...
      this.$refs[formName].validate((valid) => {
        ...
        this.$nextTick(() => {
          if (valid) {
            ...
            request.post('/externalAccount/baseData/supplierGroup/' + (this.groupEditForm.id ? 'update' : 'save'), this.groupEditForm).then(res => {
              this.$message({
    ...
    getSupplierList() {
      ...
      request.get('/externalAccount/baseData/supplier/list', {
        params: {
          currentPage: this.currentPage
          , pageSize: this.pageSize
          , total: this.total
    ...
    getMaterialGroupList() {
      request.post('/externalAccount/baseData/supplierGroup/listValide').then(res => {
        this.$nextTick(() => {
    ...
    edit(id) {
      this.addOrUpdate = "update"
      request.get('/externalAccount/baseData/supplier/queryById?id=' + id).then(res => {
    ...
    del(id) {
      ...
      request.post('/externalAccount/baseData/supplier/del', ids).then(res => {
        this.$message({
          message: '删除成功!',
          type: 'success'
        });
    ...
    del2() {
      ...
      request.get('/externalAccount/baseData/supplierGroup/delById?id=' + delId+'&&groupCode='+this.lastClickTreeNode.code).then(res => {
        this.$message({
          message: '删除成功!',
          type: 'success'
        });
