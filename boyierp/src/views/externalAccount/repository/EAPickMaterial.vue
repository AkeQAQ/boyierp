    ...
    <el-main class="elMain_my" style="padding-top: 0;padding-right: 0px">
      ...
      <el-form :inline="true" class="demo-form-inline elForm_my" >
        ...
        <el-form-item>
          ...
          <div v-if="selectedName === 'pickUser'" :class=" 'el-input el-input--mini'" style="margin: 0 0">
            <input  @keyup.enter="search()" class="el-input__inner" style="width: 200px"  placeholder="请输入搜索内容"
                    v-model.lazy="searchStr">
            </input>
          </div>
          ...
          <!-- 列表界面-单据编号搜索 -->
          <div v-if="selectedName === 'comment'" :class=" 'el-input el-input--mini'" style="margin: 0 0">
            <input  @keyup.enter="search()" class="el-input__inner" style="width: 200px"  placeholder="请输入搜索内容"
                    v-model.lazy="searchStr">
            </input>
          </div>
        ...
        <el-popover
          ...
          <ul v-for="(item,index) in manySearchArr">
            <li>
              ...
              <el-autocomplete size="mini" v-if="item.selectField==='departmentName'"
                               style="width: 200px"
                               popper-class="my-autocomplete"
                               clearable
                               class="inline-input"
              ...
              <div :class=" 'el-input el-input--mini'" v-if="item.selectField === 'pickUser'" style="width: 200px">
                <input  class="el-input__inner"   placeholder="请输入搜索内容"
                        v-model.lazy="item.searchStr">
                </input>
              </div>
        ...
        <el-form-item v-if="hasAuth('externalAccount:repository:pickMaterial:list')">
          <el-button size="mini" icon="el-icon-plus" type="primary" v-if="hasAuth('externalAccount:repository:pickMaterial:list')"
        ...
        <el-form-item  v-if="hasAuth('externalAccount:repository:pickMaterial:list')" style="margin-left: 0">
        ...
        <el-form-item  >
          <el-dropdown   @command="expChangeImport">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-if="hasAuth('externalAccount:repository:pickMaterial:list')" command="import">导入</el-dropdown-item>
        ...
        >
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="edit(scope.row.id)"
              v-if="hasAuth('externalAccount:repository:pickMaterial:list') || (hasAuth('externalAccount:repository:pickMaterial:list') && scope.row.status != 1 )   ">{{ scope.row.status === 1 ? '编辑' : '查看' }}
            </el-button>

<!--            <el-divider direction="vertical"
                        v-if="hasAuth('externalAccount:repository:pickMaterial:save') && scope.row.status ===1   "></el-divider>

            <el-button style="padding: 0" type="text"
                       v-if="hasAuth('externalAccount:repository:pickMaterial:save')  && scope.row.status ===1   ">
              <template>
            ...
            <el-divider direction="vertical"
                        v-if="hasAuth('externalAccount:repository:pickMaterial:save') && (scope.row.status === 2 || scope.row.status === 3 )   "></el-divider>

            <el-button class="elInput_action_my" type="text" style="padding: 0"
                       v-if="hasAuth('externalAccount:repository:pickMaterial:save')  && (scope.row.status === 2 || scope.row.status === 3)   ">
              <template>
            ...
            <el-button style="padding: 0" type="text"
              v-if="hasAuth('externalAccount:repository:pickMaterial:list')  && (scope.row.status === 2 || scope.row.status === 3)   ">
            ...
            <el-button style="padding: 0" type="text"
                       v-if="hasAuth('externalAccount:repository:pickMaterial:list')  && scope.row.status ===0  ">
              <template>
            ...
            <el-divider direction="vertical"
                        v-if="hasAuth('externalAccount:repository:pickMaterial:list')  && scope.row.status ===1  "></el-divider>

            <el-button style="padding: 0" type="text"
                       v-if="hasAuth('externalAccount:repository:pickMaterial:list') && scope.row.status ===1   ">
              <template>
...
<!--            <el-input :disabled="this.editForm.status!=1"  size="mini" clearable style="width: 100px" v-model="editForm.pickUser">
          ...
          <el-form-item  label="生产序号" prop="batchId" style="padding: -20px 0 ;margin-bottom: -20px">
            <div style="width: 90px;padding-left: -30px" :class=" [(this.editForm.status!=1)? 'el-input el-input--mini is-disabled' :'el-input el-input--mini']">
              <input  class="el-input__inner"
          ...
          <el-form-item v-if="hasAuth('externalAccount:repository:buyIn:list')">
            <el-dropdown   @command="action">
              <el-button  icon="el-icon-edit-outline" size="mini" type="success">
                操作<i class="el-icon-arrow-down el-icon--right"></i>
              </el-button>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="save" v-show="hasAuth('externalAccount:repository:pickMaterial:list') && this.editForm.status===1" >
                  提交单据</el-dropdown-item>
                <el-dropdown-item command="subReturn" v-show="hasAuth('externalAccount:repository:pickMaterial:list') && (this.editForm.status===2 || this.editForm.status===3)">
                  撤销</el-dropdown-item>
                <el-dropdown-item command="addNew" v-show="hasAuth('externalAccount:repository:pickMaterial:list') ">
                  新增</el-dropdown-item>
                <el-dropdown-item command="copy" v-show="hasAuth('externalAccount:repository:pickMaterial:list') ">
                  复制</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </el-form-item>
...
<!--          <el-form-item v-if="hasAuth('externalAccount:repository:buyIn:save')">
      ...
      >
        <el-form style="width: 100%;margin-bottom: -20px;margin-top: -30px;align-items: center"
          ...
          <el-form-item  label="领料人" prop="pickUser" style="padding: 0 0 ;margin-bottom: 20px;margin-left: -30px">
...
import print from "@/views/printModule/EAprintPick";
import exportExcelCommon from"../../common/ExportExcelCommon"
import {request2} from "@/axios";
import PrintPickBatch from "@/views/printModule/EAprintPickBatch";
...
/*
  ...
  methods: {
    ...
    batchPrint(){
      ...
      request.post('/externalAccount/repository/pickMaterial/getBatchPrintByIds',ids).then(res => {
    ...
    statusPassBatch(){
      let ids = this.multipleSelection;
      request.post('/externalAccount/repository/pickMaterial/statusPassBatch',ids).then(res => {
        this.$message({
          message: '审核通过!',
          type: 'success'
        });
    ...
    async handleInputConfirm() {
      ...
      await request.post('/tag/save?tagName='+inputValue+'&&type='+3+
    ...
    dialogOpend(){
      if(this.editForm.id != '' && this.editForm.id != undefined){
        ...
        request.get('/externalAccount/repository/pickMaterial/lockById?id=' + this.editForm.id)
      }
    },
    // 导出列表数据- 服务端写出字节流到浏览器，进行保存
    downImportDemo() {

      request2.post('/externalAccount/repository/pickMaterial/down',null,{responseType:'arraybuffer'}).then(res=>{
        // 这里使用blob做一个转换
    ...
    uploadRequest(fileobj) {
      ...
      request({
        method: 'post',
        url: '/externalAccount/repository/pickMaterial/upload?pickUser=' + this.editImportForm.pickUser + "&&departmentId="
    ...
    exportCurrent() {
      ...
      request2.post('/externalAccount/repository/pickMaterial/export?currentPage='+this.currentPage+
        ...
        this.saveFile(blob,'生产领料当前页.xlsx')
      }).catch()
    },
    // 导出列表数据- 服务端写出字节流到浏览器，进行保存
    exportAll() {

      request2.post('/externalAccount/repository/pickMaterial/export?'+
          "searchStartDate="+this.searchStartDate+
          "&&searchEndDate="+this.searchEndDate+
          "&&searchField="+this.select
          ,{'manySearchArr':this.manySearchArr,'searchStr':this.searchStr},{responseType:'arraybuffer'}).then(res=>{
    ...
    loadDepartmentAll() {
      request.post('/externalAccount/baseData/department/getSearchAllData').then(res => {
    ...
    loadTableSearchMaterialDetailAll() {
      request.post('/externalAccount/baseData/material/loadTableSearchMaterialDetailAll').then(res => {
...
/*
    ...
    submitForm(formName, methodName) {
      this.$refs[formName].validate((valid) => {
        ...
        if (valid) {
          ...
          request.post('/externalAccount/repository/pickMaterial/' + methodName, this.editForm).then(res => {
            load.close()
            this.$message({
    ...
    getPickDocumentList() {
      ...
      let url = '/externalAccount/repository/pickMaterial/list?currentPage='+this.currentPage+
          "&&pageSize="+this.pageSize+
          "&&total="+this.total+
          "&&searchStartDate="+this.searchStartDate+
          "&&searchEndDate="+this.searchEndDate+
    ...
    edit(id) {
      this.addOrUpdate = "update"
      request.get('/externalAccount/repository/pickMaterial/queryById?id=' + id).then(res => {
    ...
    queryByProduceBatchId(zcPickId) {
      request.get('/externalAccount/repository/pickMaterial/queryByProduceBatchInId?id=' + zcPickId).then(res => {
        let result = res.data.data
        this.dialogVisible = true
        // 弹出框我们先让他初始化结束再赋值 ，不然会无法重置
        this.$nextTick(() => {
    ...
    queryByBuyInId(id,buyInDate) {
      request.get('/externalAccount/repository/pickMaterial/queryByBuyInId?buyInId=' + id).then(res => {
        let result = res.data.data
        this.dialogVisible = true
        // 弹出框我们先让他初始化结束再赋值 ，不然会无法重置
        this.$nextTick(() => {
    ...
    del(id) {
      ...
      request.post('/externalAccount/repository/pickMaterial/del', ids).then(res => {
        load.close()
        this.$message({
          message: '删除成功!',
          type: 'success'
    ...
    // 撤销提交
    statusSubReturn(id) {
      request.get('/externalAccount/repository/pickMaterial/statusSubReturn?id=' + id).then(res => {
        this.$message({
          message: '已撤销!',
          type: 'success'
        });
    ...
    // 状态提交
    statusSubmit(id) {
      request.get('/externalAccount/repository/pickMaterial/statusSubmit?id=' + id).then(res => {
        this.$message({
          message: '已提交!',
          type: 'success'
        });
    ...
    // 状态待审核
    statusPass(id) {
      request.get('/externalAccount/repository/pickMaterial/statusPass?id=' + id).then(res => {
        this.$message({
          message: '审核通过!',
          type: 'success'
        });
    ...
    // 状态反审核
    statusReturn(id) {
      request.get('/externalAccount/repository/pickMaterial/statusReturn?id=' + id).then(res => {
        this.$message({
          message: '反审核完成!',
          type: 'success'
        });
    ...
    closeMethod(){
      if(this.editForm.id != '' && this.editForm.id != undefined){
        ...
        request.get('/externalAccount/repository/pickMaterial/lockOpenById?id=' + this.editForm.id)
      }
    ...
    toBuyIn(){
        ...
        this.$router.push({name:'externalAccount:repository:pickMaterial:list'});
    },
    ...
    async closeBrowser(){
      if(this.editForm.id != '' && this.editForm.id != undefined){
        console.log("关闭编辑页面.打开锁...",this.editForm.id);
        await request.get('/externalAccount/repository/pickMaterial/lockOpenById?id=' + this.editForm.id)
      }
    },
