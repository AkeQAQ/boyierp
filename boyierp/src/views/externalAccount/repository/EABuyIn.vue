    ...
    <el-main class="elMain_my" style="padding-top: 0;padding-right: 0px">
      ...
      <el-form :inline="true" class="demo-form-inline elForm_my" >
        ...
        <el-form-item>
          ...
          <div v-if="selectedName === 'supplierDocNum'" :class=" 'el-input el-input--mini'" style="margin: 0 0">
            <input  @keyup.enter="search()" class="el-input__inner" style="width: 200px"  placeholder="请输入搜索内容"
                    v-model.lazy="searchStr">
            </input>
          </div>
          ...
          <div v-if="selectedName === 'orderSeq'" :class=" 'el-input el-input--mini'" style="margin: 0 0">
            <input  @keyup.enter="search()" class="el-input__inner" style="width: 200px"  placeholder="请输入搜索内容"
                    v-model.lazy="searchStr">
            </input>
          </div>
        ...
        <el-popover
          ...
          <ul v-for="(item,index) in manySearchArr">
            <li>
              ...
              <div :class=" 'el-input el-input--mini'" v-if="item.selectField === 'supplierDocNum'" style="width: 200px">
                <input  class="el-input__inner"   placeholder="请输入搜索内容"
                        v-model.lazy="item.searchStr">
                </input>
              </div>
              ...
              <div :class=" 'el-input el-input--mini'" v-if="item.selectField === 'orderSeq'" style="width: 200px">
                <input  class="el-input__inner"   placeholder="请输入搜索内容"
                        v-model.lazy="item.searchStr">
                </input>
              </div>
...
<!--          <el-dropdown  @command="searchDropDown" size="mini" icon="el-icon-search"  split-button type="success" @click="search">
        ...
        <el-form-item v-if="hasAuth('externalAccount:repository:buyIn:list')">
          <el-button size="mini" icon="el-icon-plus" type="primary" v-if="hasAuth('externalAccount:repository:buyIn:list')"
        ...
        <el-form-item v-if="hasAuth('externalAccount:repository:buyIn:list')" style="margin-left: 0">
        ...
        <el-form-item >
          <el-dropdown   @command="otherBtn">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item v-if="hasAuth('externalAccount:repository:buyIn:list')" command="orderBatchPick">订单一键领料</el-dropdown-item>
...
<!--        <el-button size="mini" icon="el-icon-share" type="primary"
        ...
        <!--        <el-form-item v-if="hasAuth('externalAccount:repository:buyIn:del')">
        ...
        >
          <template slot-scope="scope"   >
            <el-button type="text" size="small"
                       @click="hasAuth('externalAccount:repository:buyIn:list') && edit(scope.row.id)"
            >{{ scope.row.id }}
            </el-button>
          </template>
...
<!--          <template slot-scope="scope">
            <el-button type="text" size="small"
                       @click="hasAuth('externalAccount:repository:buyIn:update') && edit(scope.row.id)"
        ...
        >
          <template slot-scope="scope">
            <el-button class="elInput_action_my" type="text" size="small" @click="edit(scope.row.id)"  style="padding: 0"
                       v-if="hasAuth('externalAccount:repository:buyIn:list') || (hasAuth('externalAccount:repository:buyIn:list') && scope.row.status != 1 )   ">{{ scope.row.status === 1 ? '编辑' : '查看' }}
            </el-button>

<!--            <el-divider direction="vertical"
                        v-if="hasAuth('externalAccount:repository:buyIn:save') && scope.row.status === 1   "></el-divider>

            <el-button class="elInput_action_my" type="text" style="padding: 0"
                       v-if="hasAuth('externalAccount:repository:buyIn:save')  && scope.row.status === 1   ">
              <template>
            ...
            <el-divider direction="vertical"
                        v-if="hasAuth('externalAccount:repository:buyIn:save') && (scope.row.status === 2 || scope.row.status === 3 )   "></el-divider>

            <el-button class="elInput_action_my" type="text" style="padding: 0"
                       v-if="hasAuth('externalAccount:repository:buyIn:save')  && (scope.row.status === 2 || scope.row.status === 3)   ">
              <template>
...
<!--            <el-divider direction="vertical"
                        v-if="hasAuth('externalAccount:repository:buyIn:valid') && (scope.row.status === 2 || scope.row.status === 3)  "></el-divider>-->

            <el-button class="elInput_action_my" type="text" style="padding: 0"
                       v-if="hasAuth('externalAccount:repository:buyIn:list')  && (scope.row.status === 2 || scope.row.status === 3)   ">
              <template>
            ...
            <el-divider direction="vertical"
                        v-if="hasAuth('externalAccount:repository:buyIn:list')  && scope.row.status === 0  "></el-divider>

            <el-button class="elInput_action_my" type="text"  style="padding: 0"
                       v-if="hasAuth('externalAccount:repository:buyIn:list')  && scope.row.status === 0  ">
              <template>
            ...
            <el-divider direction="vertical"
                        v-if="hasAuth('externalAccount:repository:buyIn:list')  && scope.row.status ===1  "></el-divider>

            <el-button class="elInput_action_my" type="text"  style="padding: 0"
                       v-if="hasAuth('externalAccount:repository:buyIn:list') && scope.row.status ===1   ">
              <template>
        ...
        <el-form
          ...
          <el-form-item v-if="hasAuth('externalAccount:repository:buyIn:list')">
            <el-dropdown   @command="action">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="save" v-show="hasAuth('externalAccount:repository:buyIn:list') && this.editForm.status===1" >
                  提交单据</el-dropdown-item>
                <el-dropdown-item command="subReturn" v-show="hasAuth('externalAccount:repository:buyIn:list') && (this.editForm.status===2 || this.editForm.status===3)">
                  撤销</el-dropdown-item>
                <el-dropdown-item command="addNew" v-show="hasAuth('externalAccount:repository:buyIn:list') ">
                  新增</el-dropdown-item>
                <el-dropdown-item command="copy" v-show="hasAuth('externalAccount:repository:buyIn:list') ">
                  复制</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </el-form-item>
  ...
  methods: {
    batchPickDone(){
      const load = this.$loading({
       ...
       request.get('/externalAccount/repository/buyIn/batchPick?startDateStr='+this.editBatchPickForm.startDate
    ...
    changeNum(seq,supplierId,materialId,buyIndate){
      ...
      if(this.editForm.rowList[seq-1].price === ''){
        request.get('/externalAccount/baseData/supplierMaterial/queryByValidPrice?supplierId='+supplierId+'&&materialId='+materialId
    ...
    loadSupplierAll() {
      request.post('/externalAccount/baseData/supplier/getSearchAllData').then(res => {
    ...
    exportCurrentList() {
      ...
      let checkStr = this.checkedBox.join(",");
      request2.post('/externalAccount/repository/buyIn/export?currentPage='+this.currentPage+
    ...
    exportAll() {
      ...
      let checkStr = this.checkedBox.join(",");
      request2.post('/externalAccount/repository/buyIn/export?'+
          "searchStartDate="+this.searchStartDate+
          "&&searchEndDate="+this.searchEndDate+
          "&&searchField="+this.select+
          "&&searchStatus="+checkStr
      ...
      this.$refs[formName].validate((valid) => {
        ...
        if (valid) {
          ...
          const load = this.$loading({
           ...
           request.post('/externalAccount/repository/buyIn/' + methodName, this.editForm).then(res => {
             load.close()
            this.$message({
    ...
    async getBuyInDocumentList() {
      ...
      await request.post('/externalAccount/repository/buyIn/list?currentPage='+this.currentPage+
          "&&pageSize="+this.pageSize+
          "&&total="+this.total+
          "&&searchStartDate="+this.searchStartDate+
          "&&searchEndDate="+this.searchEndDate+
    ...
    edit(id) {
      this.addOrUpdate = "update"
      request.get('/externalAccount/repository/buyIn/queryById?id=' + id).then(res => {
    ...
    statusPassBatch(){
      ...
      request.post('/externalAccount/repository/buyIn/statusPassBatch',ids).then(res => {
        this.$message({
          message: '审核通过!',
          type: 'success'
        });
    ...
    del(id) {
      ...
      request.post('/externalAccount/repository/buyIn/del', ids).then(res => {
        load.close()
        this.$message({
          message: '删除成功!',
          type: 'success'
    ...
    // 撤销
    statusSubReturn(id) {
      request.get('/externalAccount/repository/buyIn/statusSubReturn?id=' + id).then(res => {
        this.$message({
          message: '已撤销!',
          type: 'success'
        });
    ...
    // 状态提交
    statusSubmit(id) {
      request.get('/externalAccount/repository/buyIn/statusSubmit?id=' + id).then(res => {
        this.$message({
          message: '已提交!',
          type: 'success'
        });
    ...
    // 状态待审核
    statusPass(id) {
      request.get('/externalAccount/repository/buyIn/statusPass?id=' + id).then(res => {
        this.$message({
          message: '审核通过!',
          type: 'success'
        });
    ...
    // 状态反审核
    statusReturn(id) {
      request.get('/externalAccount/repository/buyIn/statusReturn?id=' + id).then(res => {
        this.$message({
          message: '反审核完成!',
          type: 'success'
        });
    ...
    closeMethod(){
      if(this.editForm.id != '' && this.editForm.id != undefined){
        ...
        request.get('/externalAccount/repository/buyIn/lockOpenById?id=' + this.editForm.id)
      }
    ...
    batchPrint(){
      ...
      let ids = this.multipleSelection;
      request.post('/externalAccount/repository/buyIn/getBatchPrintByIds',ids).then(res => {
    ...
    dialogOpend(){
      if(this.editForm.id != '' && this.editForm.id != undefined){
        ...
        request.get('/externalAccount/repository/buyIn/lockById?id=' + this.editForm.id)
      }
    },
    pickMaterial(){
      if(this.editForm.id != ''){
        ...
        this.$router.push({name:'externalAccount:repository:pickMaterial:list',params:{id:pickId,buyInDate:buyInDate}});
      }else {
    ...
    async closeBrowser(){
      if(this.editForm.id != '' && this.editForm.id != undefined){
        console.log("关闭编辑页面.打开锁...",this.editForm.id);
        await request.get('/externalAccount/repository/buyIn/lockOpenById?id=' + this.editForm.id)
      }
    },
